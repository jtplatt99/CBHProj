...
! then rightward)
! This program handles colors specifically
!
...
!* columnWidth = recordLength+6     numberInRow ~ nRecs / 3
  INTEGER,PARAMETER::columnWidth=31,numberInRow=15,recordLength=25
...
!* For other programs change the file location
  OPEN(10,FILE='cbhprojDB/color.db',FORM='FORMATTED',ACCESS='DIRECT',RECL=recordLength,ACTION='READ')
...
!*   Take note: Depending on the lngth of the code, you may have to alter the offsets written below
    ...
    line(rowPos)(characterPos+4:characterPos+6)=data(1:3)
    line(rowPos)(characterPos+8:characterPos+columnWidth-2)=data(4:recordLength)
...
!* For other programs replace color
  WRITE(*,*) "* * * Police Information System Color Database * * *"
  WRITE(*,*)
